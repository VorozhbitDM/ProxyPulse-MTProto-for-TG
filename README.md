...
- два режима ленты: **прямой** (t.me) и **обходной** (TGStat + web.archive.org);
- вход в **TGStat** через Telegram-бота @tg_analytics_bot для полной пагинации;
...
|:---:|:---:|
| ![Стартовый экран](docs/Main.jpg) | ![Результаты поиска](docs/work.jpg) |
...
|----------|--------------|----------|
| **Источник ленты** | Обходной (TGStat + Archive) | Прямой путь (t.me) или обходной (TGStat + archive.org). |
| **TGStat** | — | Вход через бота @tg_analytics_bot для полной ленты (пагинация). |
| **Лимит прокси** | `100` | Сколько уникальных прокси собрать за один поиск (от 10 до 500). |
| **Тема** | Белая | Переключатель «Тёмная» / «Белая» в настройках. |

Настройки сохраняются в `%AppData%\ProxyPulse\settings.txt` и подхватываются при следующем запуске.
...
Готовый файл: `dist\ProxyPulse.exe` и архив `dist\ProxyPulse-v2.9-win-x64.zip` (внутри только exe).
